...
async function proxy(request) {
  ...
  try {
    ...
    // Expecting: /hls/{base64}
    if (!pathname.startsWith('/hls/')) {
      return new Response('"url" path parameter required. Use /hls/{base64}', {
        status: 400,
        headers: { 'Access-Control-Allow-Origin': '*' },
      });
    }

    const encodedUrl = pathname.replace('/hls/', '');
    ...
    const isM3U8 = m3u8ContentTypes.some((type) =>
      contentType.includes(type)
    );

    // 🔹 If NOT m3u8 → direct stream return
    if (!isM3U8) {
    ...
    responseContent = responseContent.replace(/URI=["'](.*?)["']/g, (_, keyUrl) => {
      const fullUrl = keyUrl.startsWith('http')
        ? keyUrl
        : keyUrl.startsWith('/')
        ? `${baseUrl.protocol}//${baseUrl.host}${keyUrl}`
        : `${basePath}${keyUrl}`;

      return `URI="${url.origin}/hls/${encodeURIComponent(btoa(fullUrl))}"`;
    });

    // Rewrite segment URLs
    const modifiedBody = responseContent.replace(
      /^(?!#)([^\s]+)$/gm,
      (match) => {
        ...
        return `${url.origin}/hls/${encodeURIComponent(btoa(fullUrl))}`;
      }
    );
